...
{
    // Let's take  a look at how C# manages Threads (OS Threads not CPU threads.)
    ...
    Console.WriteLine($"During thread delegate running, isAlive = {workerThread.IsAlive}"); // Unstarted
    ...
    // Interleaving - Below even the runtime the actuall OS scheduler (The thing the kernel uses to map
    ...
    // Let's just use a Loop to create a few really fast
    for (int i = 1; i <= 5; i++)
    {
        int id = i;
        var th = new Thread(() =>
    ...
    // Let's make a ConcurrentQueue for FIFO work, we'll just have it store ints
    ...
    // But it's very low level. You manage each thread, you can't return a value in a straightforward way,
    ...
    static void ParallelSum()
    {
        ...
        // First - Let's do this totally sequentially - one thread without tasks
        ...
        // Before we parallelize this, let's play with Tasks
        // Manually splitting the summing into two tasks, each gets half the total numbers
        // Using two tasks provides a similar and more straightforward approach
        ...
        // Let's parallelize this with Tasks ans the TPL Library
        ...
        Parallel.For(0, data.Length,
            // After we givr it start and values for the loop - this is a For Loop
    ...
    static void RaceDemo()
    {
        ...
        // Our balance is wrong every time - and it's a defferent wrong answer every time
    ...
    {
        var bank = new Bank();
        Parallel.For(0,100000,_ => bank.DepositSafe(1)); // 100k threads worth of + 1
        Console.WriteLine($"Unsafe balance = {bank.Balance} (expected 1000000)");
        // Our balance is wrong every time - and it's a defferent wrong answer every time
        // This is the worst kind of bug. Because it's not deterministic
    }
    ...
    // Starvation - A thread gets blocked by another threads work - and stays alive but cannot progress
    // Different from a deadlock - because the other thread is able to resolve
    ...
    // Reather than abruptly killing a thread or having it dia via some exception
    ...
    static void CancellationDemo()
    {
        ...
        {
            Console.WriteLine("Work has cancelled cooperatively");
        }
    ...
    static void ExceptionDemo()
    {
        ...
        // We'd imagine that line 279 is where the exception is thrown. It's actually
    ...
    // Async / awit - related to but not the same as a thread.
    await AsyncDemo();
