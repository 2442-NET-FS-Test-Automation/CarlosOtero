...
export function BookCard({item, compact = false}: BookCardProps){
    ...
    // Components return JSX (in our case TSX): HTML lie syntax,
    ...
    return(
        <article className="card">
            ...
            <dl>
                <dt>SKU</dt>
                <dd>{item.sku}</dd>
                {/*Here we conditionally render based on the value of compact*/}
                {!compact && (
                    <>
                        <dt>In Stock</dt>
                        <dd className={item.currentStock === 0 ? "out" : ""}>{item.currentStock}</dd>
                    </>
                )}
            </dl>
        </article>
